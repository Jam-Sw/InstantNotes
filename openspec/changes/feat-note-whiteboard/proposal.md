...
right unit is still a note: permanently convert a note into a whiteboard, so
capture, library, tags, and spaces stay the same.
...
- Whiteboard engine payload lives in `surfaceData` (JSON)
- One-way convert: document → whiteboard, always danger-confirmed; no convert-back
- Command palette: permanent convert for the selected document note
...
- No convert-back from whiteboard to document
